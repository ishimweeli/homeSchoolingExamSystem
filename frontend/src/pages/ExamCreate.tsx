  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      <div className="max-w-7xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-800 mb-2">Create Exam</h1>
          <p className="text-gray-600">AI-powered exam generation with multiple question types</p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Topics to Cover * (comma-separated)</label>
                  <textarea
                    name="topics"
              ...
              <div>
                <h3 className="text-lg font-semibold mb-4">
                  ...
                  <span className="text-sm font-normal text-gray-500 ml-2">(Total: {getTotalQuestions()} questions)</span>
                </h3>
                  ...
                  {questionTypes.map(qt => (
                    <div key={qt.type} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        <button type="button" onClick={() => updateQuestionTypeCount(qt.type, -1)} className="w-8 h-8 rounded-full bg-white border border-gray-300 flex items-center justify-center hover:bg-gray-100">-</button>
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                  <p className="text-sm text-blue-700"><strong>AI Generation:</strong> Our AI will create diverse, grade-appropriate questions based on your topics and preferences.</p>
                </div>
              </div>
            </div>

            <div className="mt-8 flex justify-end space-x-4">
              <button onClick={() => navigate('/exams')} className="px-6 py-3 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">Cancel</button>
              <button onClick={generateExam} disabled={generating || getTotalQuestions() === 0} className="px-8 py-3 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold hover:shadow-lg transform hover:scale-105 transition disabled:opacity-50 disabled:cursor-not-allowed">
                {generating ? (
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex space-x-2">
                ...
                <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">{generatedExam.questions?.length || 0} Questions</span>
              </div>
            </div>

            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              <div className="grid grid-cols-4 gap-4 text-sm">
                <div><span className="text-gray-600">Title:</span><p className="font-semibold">{generatedExam.title}</p></div>
                <div><span className="text-gray-600">Subject:</span><p className="font-semibold">{generatedExam.subject}</p></div>
                <div><span className="text-gray-600">Grade:</span><p className="font-semibold">Grade {generatedExam.gradeLevel}</p></div>
                <div><span className="text-gray-600">Duration:</span><p className="font-semibold">{generatedExam.duration} minutes</p></div>
              </div>
            </div>
              ...
              {generatedExam.questions?.map((q: any, index: number) => (
                <div key={index} className="border rounded-lg p-4 hover:shadow-md transition">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center space-x-3">
                      <span className="flex items-center justify-center w-8 h-8 bg-blue-100 text-blue-600 rounded-full font-semibold">{index + 1}</span>
                      <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">{q.type.replace('_', ' ')}</span>
                      <span className={`px-2 py-1 rounded text-xs ${q.difficulty === 'EASY' ? 'bg-green-100 text-green-700' : q.difficulty === 'MEDIUM' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>{q.difficulty}</span>
                      <span className="text-sm text-gray-500">{q.marks} marks</span>
                    </div>
                    <button onClick={() => setEditingQuestion(editingQuestion === index ? null : index)} className="text-blue-600 hover:text-blue-800">{editingQuestion === index ? 'Done' : 'Edit'}</button>
                  </div>

                  {editingQuestion === index ? (
                    <div className="space-y-2">
                      <textarea value={q.question} onChange={(e) => updateQuestion(index, 'question', e.target.value)} className="w-full px-3 py-2 border rounded-lg" rows={2} />
                      ...
                      <input type="text" value={Array.isArray(q.correctAnswer) ? q.correctAnswer.join(', ') : typeof q.correctAnswer === 'object' ? JSON.stringify(q.correctAnswer) : q.correctAnswer} onChange={(e) => updateQuestion(index, 'correctAnswer', e.target.value)} className="w-full px-3 py-1 border rounded" placeholder="Correct answer" />
                    </div>
                  ) : (
                    <div>
                      ...
                      {Array.isArray(q.options) && (
                        <div className="ml-4 space-y-1">
                          {q.options.map((opt: any, i: number) => {
                            const isObject = opt && typeof opt === 'object' && !Array.isArray(opt);
                            const label = typeof opt === 'string' ? opt : isObject && (opt.active || opt.passive) ? `${opt.active ?? ''} — ${opt.passive ?? ''}`.trim() : JSON.stringify(opt);
                            return (<p key={i} className="text-sm text-gray-600">{String.fromCharCode(65 + i)}. {label}</p>);
                          })}
                        </div>
                      )}
                      <p className="text-sm text-green-600 mt-2">Answer: {Array.isArray(q.correctAnswer) ? q.correctAnswer.join(', ') : typeof q.correctAnswer === 'object' ? JSON.stringify(q.correctAnswer) : q.correctAnswer}</p>
                    </div>
                  )}
                </div>
              ))}
